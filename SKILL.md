...
**预览生成要求：**
- 保存至用户桌面的 `~/Desktop/xhs-previews/style-a.html`、`style-b.html`、`style-c.html`（**不要放在 .claude-design 下**，IDE 会意外清空）
- 每个预览必须是 **3:4 比例卡片**（使用 `card-base.css` 规范），展示真实字体/颜色/排版/入场动效
- 预览内容用实际的用户内容（封面卡），不用假占位符
- 每个预览 ~50-100 行，自包含
- 生成完后**逐个 `open` 打开**（三个都要打开）：
  ```bash
  open ~/Desktop/xhs-previews/style-a.html
  open ~/Desktop/xhs-previews/style-b.html
  open ~/Desktop/xhs-previews/style-c.html
  ```
...
1. **清理临时文件** — 删除 `~/Desktop/xhs-previews/` 目录
...
> 卡片已生成！如果你想在浏览器里改改文字，现在可以去调整。
> 改完之后告诉我「**导出图片**」，我会自动把每张卡片截图保存为 PNG，直接可上传小红书 🌿

**⚠️ 重要：不要主动执行导出。必须等用户明确说「导出图片」「截图」「下载图片」等确认词后，才进入 Phase 5。**

---

## Phase 5: 导出图片（等用户确认后执行）

> 触发条件：用户说「导出图片」「截图」「下载」「保存图片」等确认词。

### Step 5.1: 确认 HTML 文件路径

**默认直接使用 Phase 3 生成的 HTML 文件路径，不需要询问。**

只有以下两种情况才询问用户路径：
1. 用户在对话中明确提到「我改了文件」「我保存了新版本」「⌘S 导出了」等，说明存在更新版本
2. Phase 3 生成时文件路径未被记录（如跨对话触发 Phase 5）

询问方式：
> 你用的是原始生成的 HTML，还是 ⌘S 导出的修改版？如果是修改版，把文件路径告诉我。

### Step 5.2: 确认图片输出目录

默认在 HTML 文件同级目录下创建 `images/` 子文件夹，如：
- HTML：`~/Desktop/my-cards.html`
- 图片输出：`~/Desktop/images/card-01.png` … `card-N.png`

### Step 5.3: 写出截图脚本并执行

在 HTML 文件同级目录创建临时脚本 `_screenshot.mjs`，内容如下（变量替换为实际值）：
...
const htmlFile = '/absolute/path/to/cards.html';   // ← 替换为实际路径
const outDir   = path.join(path.dirname(htmlFile), 'images');
const CARD_W   = 1080;
const CARD_H   = 1440;

// 自动检测卡片总数（从 HTML 中读取）
const html = fs.readFileSync(htmlFile, 'utf8');
const matches = html.match(/id="card-\d+"/g) || [];
...
(async () => {
  ...
  for (let i = 1; i <= TOTAL; i++) {
    const cardId = `card-${i}`;
    console.log(`📸 截图卡片 ${i}/${TOTAL}...`);
    await page.evaluate((id) => {
      const el = document.getElementById(id);
      if (el) el.scrollIntoView({ behavior: 'instant' });
    }, cardId);
    await new Promise(r => setTimeout(r, 1000));
    const cardEl = await page.$(`#${cardId}`);
    if (!cardEl) { console.warn(`  ⚠️ 找不到 #${cardId}，跳过`); continue; }
    const filename = path.join(outDir, `card-${String(i).padStart(2,'0')}.png`);
    await cardEl.screenshot({ path: filename, type: 'png' });
...
**在专用临时目录安装 puppeteer，不污染用户项目：**

```bash
# 1. 创建独立临时目录安装依赖（避免污染 HTML 所在目录）
mkdir -p /tmp/xhs-screenshot && cd /tmp/xhs-screenshot
npm init -y
npm install puppeteer

# 2. 将脚本拷贝过来执行
cp [html文件目录]/_screenshot.mjs /tmp/xhs-screenshot/
node _screenshot.mjs
```

> **⚠️ 注意事项：**
> - puppeteer 首次安装需下载 Chromium（约 150MB），网络慢时需等待 2-5 分钟
> - 若 Chromium 下载失败，脚本中 `executablePath` 已默认指向本机 Chrome（`/Applications/Google Chrome.app/...`），Mac 上通常直接可用，无需额外下载
> - `/tmp/xhs-screenshot` 目录系统重启后自动清理，无需手动删除；若 puppeteer 已安装过，可跳过 `npm init / npm install` 步骤直接执行

### Step 5.5: 完成后清理 + 打开图片文件夹

```bash
rm [html文件目录]/_screenshot.mjs   # 删除 HTML 目录中的临时脚本
open [outDir]                        # 打开图片文件夹
```

**汇报信息：**
- 图片保存路径
- 共 N 张，分辨率 2160×2880px（@2x，对应 1080×1440 逻辑尺寸）
- 直接可上传小红书
