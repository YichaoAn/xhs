...
使用 Phase 3 生成的 HTML 文件路径（若用户通过 `⌘S` 修改导出了新文件，询问用户当前最新的 HTML 文件路径）。
...
> **⚠️ puppeteer 首次安装需下载 Chromium（约 150MB），网络慢时需等待 2-5 分钟。**
> 若下载失败（网络问题），改用本机 Chrome：脚本中 `executablePath` 已默认指向
> `/Applications/Google Chrome.app/Contents/MacOS/Google Chrome`，Mac 上通常直接可用。

### Step 5.5: 完成后清理 + 打开图片文件夹

```bash
rm _screenshot.mjs          # 删除临时脚本
open [outDir]               # 打开图片文件夹
```
